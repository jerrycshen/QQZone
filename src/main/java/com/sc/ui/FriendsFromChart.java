...
public class FriendsFromChart {

	public FriendsFromChart(ViewChartService chartService) {
		initFriendsFromChart(chartService);
	}

	private void initFriendsFromChart(ViewChartService chartService) {

		Map<String, Integer> map = chartService.viewAddressRatio();
		...
		JFreeChart chart = ChartFactory.createBarChart3D("你的好友大多数来自...", "好友昵称",
				"人次", dataset, PlotOrientation.VERTICAL, false, true, true);
		ChartFrame frame = new ChartFrame("你的好友大多数来自...", chart, true);
		// 自定义设定背景色
...
//		chart.getLegend().setItemFont(new Font("黑体", 0, 16));
		...
		TextTitle title = new TextTitle("Where are you from my friends？");
