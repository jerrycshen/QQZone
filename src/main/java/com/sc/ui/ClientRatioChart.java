...
public class ClientRatioChart {
	
	public ClientRatioChart(ViewChartService chartService) {
	...
	private void initClientRatioChart(ViewChartService chartService) {
		...
		Map<String, Integer> map = chartService.viewShuoShuoClientChart();
		// 创建饼图数据对象
		...
		JFreeChart chart = ChartFactory.createPieChart3D("发表说说使用的客户端占比",
				dfp, true, true, true);
		// 图片背景色
		chart.setBackgroundPaint(Color.white);
		// 设置标题文字
		ChartFrame frame = new ChartFrame("发表说说使用的客户端占比", chart, true);
		// 取得3D饼图对象
        ...
        TextTitle title = new TextTitle("土豪你换了多少手机？");  
        title.setFont(font);  
