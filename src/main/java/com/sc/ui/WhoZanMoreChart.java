...
public class WhoZanMoreChart {

	public WhoZanMoreChart(ViewChartService chartService) {
		initZanMoreChart(chartService);
	}

	private void initZanMoreChart(ViewChartService chartService) {

		Map<String, Integer> map = chartService.viewWhoZanMore();

		// 创建饼图数据对象
		DefaultCategoryDataset dataset = new DefaultCategoryDataset();
		Set<Entry<String, Integer>> set = map.entrySet();
		for (Entry<String, Integer> entry : set) {
			dataset.setValue(entry.getValue(), "赞数量",entry.getKey());
		}
		
		JFreeChart chart = ChartFactory.createBarChart3D("赞次数TOP10", "好友昵称",
				"赞数量", dataset, PlotOrientation.VERTICAL, false, true, true);
		ChartFrame frame = new ChartFrame("赞次数TOP10", chart, true);
		// 自定义设定背景色
...
//		chart.getLegend().setItemFont(new Font("黑体", 0, 16));
		...
		TextTitle title = new TextTitle("谁是赞你最多的人？");
