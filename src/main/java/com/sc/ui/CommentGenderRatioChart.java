...
public class CommentGenderRatioChart {
	
	public CommentGenderRatioChart(ViewChartService chartService) {
	...
	private void initClientRatioChart(ViewChartService chartService) {
		...
		Map<String, Integer> map = chartService.viewCommentGenderRatioChart();
		// 创建饼图数据对象
		...
		JFreeChart chart = ChartFactory.createPieChart3D("你收到评论的男女比例",
				dfp, true, true, true);
		// 图片背景色
		chart.setBackgroundPaint(Color.white);
		// 设置标题文字
		ChartFrame frame = new ChartFrame("你收到评论的男女比例", chart, true);
		// 取得3D饼图对象
        ...
        TextTitle title = new TextTitle("你的所有评论帅哥多还是美女多？");  
        title.setFont(font);  
